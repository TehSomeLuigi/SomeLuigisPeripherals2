...
/**
 * The <code>Appender</code> represents a packet that wraps a pooled
 ...
 * means that the <code>append</code> methods can add data to the
...
class Appender implements Packet {
   ...
   /**
    * Constructor for the <code>Appender</code> object. This creates
    * a packet with a recyclable buffer. This appender can not be
    * recycled as it is provided with no recycler. 
    * 
    * @param buffer this is the buffer used by this packet 
    * @param sequence this is the unique sequence number for this
    */
   public Appender(ByteBuffer buffer, long sequence) {
